        ...
        <div className="flex items-center gap-6">
          <div className="relative group">
            ...
            <div className="absolute right-0 mt-2 w-56 glass rounded-2xl p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform origin-top-right scale-95 group-hover:scale-100 shadow-2xl z-50">
              ...
              <button 
                onClick={onLogout}
                className="w-full flex items-center gap-3 p-3 rounded-xl text-gray-400 hover:bg-rose-500/20 hover:text-rose-500 transition-colors font-semibold text-sm"
              >
                Logout
              </button>
            </div>
      ...
      <div className="flex flex-1 pt-[72px] h-full overflow-hidden">
        ...
        `}>
          <div className="mb-8 px-4">
            ...
            <div className="space-y-2">
              <button
                ...
                className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${activeTab === 'home' ? 'bg-indigo-600 text-white shadow-lg' : 'hover:bg-white/5 text-white/60'}`}
              >
              ...
              <button
                ...
                className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${activeTab === 'advisor' ? 'bg-indigo-600 text-white shadow-lg' : 'hover:bg-white/5 text-white/60'}`}
              >
                <span className="text-xl">🤖</span>
          ...
          <div className="mt-auto p-6 rounded-3xl bg-indigo-600/10 border border-indigo-500/20 relative overflow-hidden group">
            <h3 className="font-black text-indigo-400 text-sm mb-2 uppercase italic">Premium</h3>
            <p className="text-[11px] text-white/50 font-medium leading-relaxed mb-4 italic">
              Unlock advanced tax-saving strategies and automated insights.
            </p>
            <button className="w-full py-3 bg-indigo-500 hover:bg-indigo-400 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all">
              Upgrade
            </button>
          </div>
          ...
          <div className="md:hidden mt-4 pt-4 border-t border-white/5">
            <button 
              onClick={onLogout}
              className="w-full flex items-center gap-4 px-5 py-4 rounded-2xl text-rose-500 hover:bg-rose-500/10 transition-all font-bold"
            >
              <span className="text-xl">🚪</span>
              Logout
            </button>
        ...
        {isMobileMenuOpen && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[1050] md:hidden transition-opacity" onClick={() => setIsMobileMenuOpen(false)} />
        )}
          ...
          <div className="max-w-5xl mx-auto w-full">
            {activeTab === 'home' && (
              <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
                <div className="relative p-8 md:p-12 rounded-[2.5rem] overflow-hidden border border-white/5 shadow-2xl">
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                    ...
                    <button 
                      ...
                      className="bg-white text-black px-8 py-4 rounded-2xl font-black text-sm uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-xl"
                    >
                      Talk to advisor
                    </button>
                  </div>
